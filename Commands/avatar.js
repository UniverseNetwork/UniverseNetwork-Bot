module.exports = {
    name: "avatar", description: "To show avatar", run(m) {
        m.delete();
        ...
        m.channel.send({
            embeds: [new MessageEmbed().setColor('RANDOM').setAuthor(g.name, g.iconURL({ dynamic: true }), 'https://minecraft-mp.com/server-s272254')
                ...
                .setImage(member.user.displayAvatarURL({ dynamic: true, size: 512 })).setFooter('Made By ARVIN3108 ID', require('../config.json').Icon)]
        })
    }
}