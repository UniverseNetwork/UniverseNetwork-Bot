module.exports = (oldState, newState) => {
    let jointocreatemap = new Map(),
        oldparentname = "unknown",
        oldchannelname = "unknown",
        oldchanelid = "unknown";
    ...
    if (oldState.channelID && newState.channelID) {
        ...
        if (oldState.channelID !== newState.channelID) {
            ...
            if (jointocreatemap.get(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`)) {
                //fetch the channel
                let vc = oldState.guild.channels.cache.get(jointocreatemap.get(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`));
                //if the size is under 1
                if (vc.members.size < 1) {
                    //delete it from the map
                    jointocreatemap.delete(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`);
                    //log it 
                    console.log(" :: " + oldState.member.user.username + "#" + oldState.member.user.discriminator + " :: Leave The Room")
                    //delete the room
                    return vc.delete();
                }
                else {
    ...
    async function jointocreatechannel(user) {
        ...
        await user.guild.channels.create(`${user.member.user.username}'s Room`, {
            ...
            await vc.overwritePermissions([
                {
                    id: user.id,
                    allow: ['MANAGE_CHANNELS', 'MANAGE_ROLES', 'CONNECT', 'CREATE_INSTANT_INVITE'],
                },
                {
                    id: user.guild.id,
                    deny: ['CONNECT', 'VIEW_CHANNEL', 'CREATE_INSTANT_INVITE']
                },
                {
